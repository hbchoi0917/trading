...
def run_screener():
    ...
    logger.info(f"Tier 1 (Core) : {', '.join(TIER1_CORE)}")
    logger.info(f"Tier 2 (Watch): {', '.join(TIER2_WATCHLIST)}")
    logger.info("=" * 70)

    all_results = {}

    # Screen Tier 1 first (higher priority)
    ...
    if all_results:
        ...
        # Sort: Tier 1 first, then by Signal Strength descending
        tier_order = {'TIER1_CORE': 0, 'TIER2_WATCH': 1}
        results_df['_tier_rank'] = results_df['Tier'].map(tier_order)
        results_df = results_df.sort_values(
            ['_tier_rank', 'Signal_Strength'], ascending=[True, False]
        ...
        logger.info("\n[TOP SIGNALS — TIER 1 FIRST, BY SIGNAL STRENGTH]")
        print(results_df[['Tier', 'Signal_Strength', 'RSI', 'Price', 'BB_Position', 'ATR_%', 'Vol_Surge']])
